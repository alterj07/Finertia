  ...
  return <div className="pb-8">
    ...
    <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
      <aside className="h-fit border border-rule bg-paper-raised p-4">
        ...
        <input aria-label="Timing shift in days" type="range" min="0" max="42" step="7" value={days} onChange={(e) => setDays(Number(e.target.value))} className="mt-2 w-full accent-[var(--forecast)]" />
      ...
      <div className="min-w-0">
        ...
        <section className="mt-5 border border-rule bg-paper-raised p-3 sm:p-4">
          ...
          <div className="h-[300px] w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={scenario} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}><CartesianGrid stroke="var(--rule-soft)" vertical={false}/><XAxis dataKey="week" tickLine={false} axisLine={{ stroke: "var(--rule)" }} tick={{ fontSize: 10, fontFamily: "var(--font-mono)", fill: "var(--ink-soft)" }}/><YAxis tickLine={false} axisLine={false} width={54} tickFormatter={(v) => `$${v}K`} tick={{ fontSize: 10, fontFamily: "var(--font-mono)", fill: "var(--ink-soft)" }}/><Tooltip content={<CustomTooltip />}/><Legend iconType="plainline" wrapperStyle={{ fontSize: 11, paddingTop: 12 }}/><Line name="Base plan" dataKey="base" stroke="var(--ink-soft)" strokeWidth={1.5} strokeDasharray="4 3" dot={false} isAnimationActive/><Line name="Simulation" dataKey="scenario" stroke="var(--forecast)" strokeWidth={2.5} dot={false} activeDot={{ r: 4, fill: "var(--forecast)", stroke: "var(--paper)", strokeWidth: 2 }} isAnimationActive animationDuration={450}/></LineChart></ResponsiveContainer></div>
        </section>
        <section className="mt-5 border border-rule bg-paper-raised p-4">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-2xs uppercase tracking-[0.14em] text-forecast">Lean AI review</p><h2 className="mt-1 font-serif text-lg">Decision readout</h2><p className="mt-1 text-xs text-ink-soft">A compact scenario summary is sent only when requested.</p></div><button disabled={loading} onClick={getAdvice} className="inline-flex h-8 items-center gap-1.5 bg-ink px-3 text-xs text-paper transition-opacity hover:opacity-85 disabled:opacity-55"><Sparkles size={13}/>{loading ? "Reviewing…" : "Get recommendation"}</button></div>
